var ImageManagement = function () {
    ...
    function registerEvents() {
        ...
        $("#fileImage").on('change', function () {
            ...
            $.ajax({
                ...
                success: function (path) {
                    ...
                    ShopMart22.notify('Đã tải ảnh lên thành công!', 'success');
        ...
        $("#btnSaveImages").on('click', function () {
            var imageList = [];
            //$('#image-list').find('img').css({ "color": "red", "border": "2px solid red"});
            $.each($('#image-list').find('img').css({ "color": "red", "border": "2px solid red" }), function () {
                imageList.push($(this).attr('src'));
            });
            //$.each($('#image-list').find('div'), function (i, item) {
